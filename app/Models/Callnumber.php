...
public static function calla($callno)
{
    for($i = 0; $i < strlen($callno); $i++) // Get existing pattern in Ascii Decimal form
    {
        ...
        if (ord($callno[$i]) === 32)  { $calla[] = "_"; } // This is a space
...
public static function makeKey($callno)
{
    $amask = self::amask($callno);
...
return "$subclass*$caption_integer*$caption_decimal*$caption_date*$caption_ordinal_num*$caption_ordinal_ind*$cutter1*$cutter1_decimal
*$cutter_date*$inline_cutter*$inline_cutter_decimal*$cutter_date2*$cutter2*$cutter2_decimal*$specification";
